...
| Feature | Description |
|---|---|
| **Check skeleton** | A ready-to-go app framework -- you write the check functions that plug in |
...
### What You Will Build Toward (The Full Platform)

The full platform splits work across two types of services:

- **HuggingFace Spaces** -- where the heavy Python work lives (ifcopenshell, AI agents, check logic). Each team deploys their own Space. An orchestrator Space ties them together.
- **Cloudflare Pages** -- where the frontend lives (dashboard, 3D viewer, file upload). Fast, free, no Python needed.

| Role | Service | Analogy | What It Does |
|---|---|---|---|
| **Check agents** | HF Spaces (one per team) | Your workshop | Each team's checks + AI reasoning, deployed as a Gradio app |
| **Orchestrator** | HF Space (captains build) | The dispatcher | Calls each team's agent, aggregates results |
| **Frontend + 3D viewer** | Cloudflare Pages | The showroom | What users see -- dashboard, 3D viewer, file upload |
| **File storage** | Cloudflare R2 (S3-compatible) | Warehouse | Stores uploaded IFC models |
| **Results database** | Cloudflare D1 | Filing cabinet | Stores check results |

The exact details will be decided during the course. What matters is understanding the **roles** -- every production system needs these building blocks.
...
- **Early in the week:** You build and test everything locally on your laptop (the workshop phase)
- **Mid-week:** Teams deploy their agents to HuggingFace Spaces -- your check tool becomes accessible online
- **By the end:** The full platform is deployed -- frontend on Cloudflare, agents on HF Spaces, all connected
...
[CLOUDFLARE PAGES]  <-- Frontend: dashboard + 3D viewer
    |
    v
[ORCHESTRATOR HF SPACE]  <-- Calls team agents, aggregates results
    |         |         |
    v         v         v
[Team A]  [Team B]  [Team C]  ...  <-- Each team's HF Space
(Gradio + checks + ifcopenshell + AI agents)
    |
    v
...
**Remember:** On Day 1, only the bottom portion exists -- Python check scripts running inside a local Gradio interface. Everything above is what you build during the week.
...
|---|---|---|
| **Interface** | Gradio (Python-generated) | Custom frontend on Cloudflare Pages |
| **3D Viewer** | Basic Gradio viewer (rotate/zoom only) | Full BIM viewer (click elements, filter floors, clipping) |
| **Where checks run** | Locally inside Gradio | On HuggingFace Spaces, called by an orchestrator |
| **Data storage** | None -- results vanish when you close the app | Cloudflare D1 stores results permanently |
| **File storage** | Local hard drive | Cloudflare R2 (S3-compatible) |
