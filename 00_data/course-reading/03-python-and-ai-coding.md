...
| `google-genai` | Calls Google's Gemini AI models | Your AI connection |
| `pydantic-ai` | Builds AI agents with tools and structured output | Your agent framework |
...
### Agent Skills -- Shared Context for AI

Your startup maintains a shared **Agent Skills** repository -- a separate repo with a `SKILL.md` file that contains company standards: the result schema, architecture conventions, agent patterns. When you install this repo into your AI coding assistant, it picks up the standards automatically. Every AI assistant in the company follows the same rules without you repeating yourself every time.
...
In this course, you will use **PydanticAI** -- a framework that combines Pydantic's strict data contracts with AI agent capabilities. Your agents can have tools (functions they call), structured output (Pydantic models), and chains (agents calling other agents). The LLM behind it is **Gemini** (Google's AI model), called via the `google-genai` SDK.
